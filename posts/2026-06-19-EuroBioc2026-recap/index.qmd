...
data_2026 <- read_csv("gitexclude/eurobioc2026_countries.csv")
...
breaks <- unique(c(0, log1p(1), log1p(5), log1p(10), log1p(50), log1p(max_count)))
labels <- round(expm1(breaks))

p <- ggplot(data = world) +
  geom_sf(
    aes(fill = log_Count, text = paste(name, Count)),
    color = "white"
  ) +
  scale_fill_viridis(
    option = "magma",
